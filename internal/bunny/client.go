...
func (c *Client) GetRecord(ctx context.Context, recordID int) (*DNSRecord, error) {
	...
	case http.StatusMethodNotAllowed:
		...
		return c.getRecordByListing(ctx, recordID)
	default:
...
func (c *Client) getRecordByListing(ctx context.Context, recordID int) (*DNSRecord, error) {
	req, err := http.NewRequestWithContext(
		ctx,
		http.MethodGet,
		fmt.Sprintf("%s/dnszone/%s/records", apiBaseURL, c.zoneID),
		nil,
	)
	if err != nil {
		return nil, fmt.Errorf("build request: %w", err)
